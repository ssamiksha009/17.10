...
app.post('/api/store-project-matrix', async (req, res) => {
  try {
    ...
    // ...existing code... (inside app.post('/api/store-project-matrix', async (req, res) => { ... }))

    if (protocol === 'CDTire') {
      //  DELETE old rows for this project FIRST (avoid duplicates on re-submit)
      await db.query(`DELETE FROM ${projectTable} WHERE project_id = $1`, [projectId]);

      // Deduplicate by number_of_runs when copying from scratch table to project table
      insertQuery = `
        ...
        SELECT $1, s.number_of_runs, s.test_name, s.inflation_pressure, s.velocity, s.preload, s.camber, s.slip_angle, s.displacement, s.slip_range, s.cleat, s.road_surface, s.job, s.old_job, s.template_tydex, s.tydex_name, s.p, s.l
        FROM (
          SELECT DISTINCT ON (number_of_runs)
            number_of_runs, test_name, inflation_pressure, velocity, preload, camber, slip_angle, displacement, slip_range, cleat, road_surface, job, old_job, template_tydex, tydex_name, p, l
          FROM ${scratchTable}
      ...
      insertQuery = `
        ...
        DO UPDATE SET
          ...
          inflation_pressure = EXCLUDED.inflation_pressure,
          test_velocity = EXCLUDED.test_velocity,
          longitudinal_slip = EXCLUDED.longitudinal_slip,
          slip_angle = EXCLUDED.slip_angle,
          inclination_angle = EXCLUDED.inclination_angle,
  ...
  return new Promise((resolve) => {
    try {
      ...
      if (oldJobName && oldJobName !== '-') {
        ...
        // include scratch directory inside the command string; escape backslash for Windows path
        command = `abaqus job=${cleanJobName} oldjob=${cleanOldJobName} input=${cleanJobName}.inp scratch="D:\\temp" ask_delete=NO`;
        console.log(`Executing with dependency in ${folderName}: ${command}`);
        
        // ✅ Broadcast dependency info to user
        broadcastSse('run-status', { 
          run: cleanJobName, 
          folder: folderName, 
          status: 'running', 
          progress: 0,
          message: `Using dependency: ${cleanOldJobName}` 
        });
      } else {
        command = `abaqus job=${cleanJobName} input=${cleanJobName}.inp`;
        console.log(`Executing without dependency in ${folderName}: ${command}`);
        
        // ✅ Broadcast no dependency info
        broadcastSse('run-status', { 
          run: cleanJobName, 
          folder: folderName, 
          status: 'running', 
          progress: 0,
          message: 'Starting simulation (no dependencies)' 
        });
      }
      ...
      abaqusProcess.stdout.on('data', (data) => {
        ...
        // Parse Abaqus progress messages
        if (chunk.includes('STEP') || chunk.includes('INCREMENT')) {
          ...
          broadcastSse('run-status', { 
            ...
            message: `Processing simulation steps...` 
          });
        }
        
        if (chunk.includes('Assembly') || chunk.includes('Mesh')) {
          broadcastSse('run-status', { 
            run: cleanJobName, 
            folder: folderName, 
            status: 'running', 
            progress: 20,
            message: 'Building mesh and assembly...' 
          });
        }
        
        if (chunk.includes('Solving')) {
          broadcastSse('run-status', { 
            run: cleanJobName, 
            folder: folderName, 
            status: 'running', 
            progress: 50,
            message: 'Solving equations...' 
          });
        }
        
        if (chunk.includes('Writing output')) {
          broadcastSse('run-status', { 
            run: cleanJobName, 
            folder: folderName, 
            status: 'running', 
            progress: 85,
            message: 'Writing ODB output file...' 
          });
        }
      });
      ...
      abaqusProcess.on('close', (code) => {
        ...
        if (code === 0) {
          // ✅ Broadcast 95% - simulation complete, verifying ODB
          broadcastSse('run-status', { 
            run: cleanJobName, 
            folder: folderName, 
            status: 'verifying', 
            progress: 95,
            message: 'Simulation complete, verifying ODB file...' 
          });
